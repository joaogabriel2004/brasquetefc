...
export type MatchResult = {
  ...
  // boxscore[teamId][playerName] = stats
};
...
export async function simulateMatchAsync(
  ...
  for (let q = 1; q <= quarters; q++) {
    const startScoreA = score[teamA.id];
    const startScoreB = score[teamB.id];
      ...
      if (shotType === 'FT') {
        for (let ft = 1; ft <= 2; ft++) {
          if (randomChance(chance)) {
            ...
            stats.points += 1;
            stats.ftm++;
            events.push(`${attacker.name} (FT) marca 1 point!`);
          } else {
            events.push(`${attacker.name} (FT) desperdiça o lance livre.`);
          }
        }
      } else {
        if (randomChance(chance)) {
          ...
          stats.points += points;
          stats.fgm++;
          events.push(`${attacker.name} marca ${points} pontos com um ${shotType}!`);
          if (points === 3) stats.tpm++;
        } else {
          events.push(`${attacker.name} erra um ${shotType}.`);
        }
      }
    ...
    quarterScores[teamA.id].push(score[teamA.id] - startScoreA);
    quarterScores[teamB.id].push(score[teamB.id] - startScoreB);
  }
